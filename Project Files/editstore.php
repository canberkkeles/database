  ...
  <h3 class='display-4' align = 'center'>Fill the form below, empty fields will not be edited</h3><hr>
    ...
    <?php
        if(isset($_GET['ID'])){
            ...
            $storeadress = $myarr[0]["storeadress"];
            $storerating = $myarr[0]["storerating"];
        }
    ?>

<?php echo"<form method = 'POST' action='editstorefinal.php?ID={$storeid}'>"?>
<fieldset disabled>
  <div class="form-group">
          <label for="storeid">Store ID</label>
          <?php echo "<input type='number' class='form-control' id='storeid' name = 'id' placeholder='$storeid'>" ?>
        </div>
</fieldset>
      <div class="form-group">
          <label for="storename">Store Name</label>
          <?php echo "<input type='text' class='form-control' id='storename' name = 'name' placeholder='$storename'>" ?>
        </div>
        <div class="form-group">
          <label for="storeadress">Store Adress</label>
          <?php echo "<input type='text' class='form-control' id='storeadress' name = 'adress' placeholder='$storeadress'>" ?>
        </div>
        <div class="form-group">
          <label for="storerating">Store Rating</label>
          <?php echo "<input type='number' class='form-control' id='storerating' name = 'rating' placeholder='$storerating'>" ?>
        </div>
