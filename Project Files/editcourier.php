  ...
  <h3 class='display-4' align = 'center'>Fill the form below, empty fields will not be edited</h3><hr>
    ...
    <?php
        if(isset($_GET['ID'])){
            ...
            $courierid = $_GET['ID'];
            $check = mysqli_query($link,"SELECT * FROM courier WHERE CourierID = '$courierid' LIMIT 1");
            ...
            $couriername = $myarr[0]["CourierName"];
            $couriervehicle = $myarr[0]["CourierBrand"];

            $check1 = mysqli_query($link,"SELECT * FROM Courier C,store S WHERE C.CourierID = '$courierid' AND C.storeid = S.storeid");
...
<?php echo"<form method = 'POST' action='editcourierfinal.php?ID={$courierid}'>"?>
<fieldset disabled>
  <div class="form-group">
          <label for="courierid">Courier ID</label>
          <?php echo "<input type='number' class='form-control' id='courierid' name = 'id' placeholder='$courierid'>" ?>
        </div>
</fieldset>
      <div class="form-group">
          <label for="couriername">Courier Name</label>
          <?php echo "<input type='text' class='form-control' id='couriername' name = 'name' placeholder='$couriername'>" ?>
        </div>
        <div class="form-group">
          <label for="couriervehicle">Courier Vehicle</label>
          <?php echo "<input type='text' class='form-control' id='couriervehicle' name = 'vehicle' placeholder='$couriervehicle'>" ?>
        </div>
        <div class="form-group">
